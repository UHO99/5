...
public class RedisCouponStockTestService {
	...
	// Kafka의 decreaseStockBatch(couponId, requestedCount)와 같은 알고리즘: min(재고, 요청수)만큼만 원자적으로 승인.
	private static final String DECREASE_STOCK_BATCH_SCRIPT = "local stock = tonumber(redis.call('get', KEYS[1])) " + "if not stock then return 0 end " + "local granted = math.min(stock, tonumber(ARGV[1])) " + "if granted > 0 then " + "  redis.call('decrby', KEYS[1], granted) " + "end " + "return granted";

	private static final DefaultRedisScript<Long> BATCH_SCRIPT = new DefaultRedisScript<>(DECREASE_STOCK_BATCH_SCRIPT, Long.class);
	...
	public int decreaseStockBatch(long couponId, int batchSize) {
		Long granted = redisTemplate.execute(BATCH_SCRIPT, Collections.singletonList(stockKey(couponId)), String.valueOf(batchSize));
